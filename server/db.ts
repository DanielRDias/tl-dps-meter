...
export interface SharedLog {
  ...
  duration: number;
  timestamp: number;
  logData: string; // JSON stringified combat log entries
  createdAt: Date;
}
...
class Database {
  ...
  saveLog(shareId: string, playerName: string, totalDamage: number, dps: number, duration: number, timestamp: number, logData: string): Promise<SharedLog> {
